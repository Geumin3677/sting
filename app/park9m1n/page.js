...
export default function Home() {
  ...
  async function allowNoti() {
    ...
    registration.pushManager
      ...
      .then(async (subscription) => {
        const res = await axios.post('//35.221.213.98:3001/api/sub/park9m1n', {
  ...
  async function getData(){
    const res = await axios.post('//35.221.213.98:3001/api/getData', {
      id: "park9m1n"
    })
    if(res.status === 200)
    {
  ...
  async function sting() {
    const res = await axios.post('//35.221.213.98:3001/api/sting/park9m1n')
    if(res.status === 200)
    {
      setCnt(cnt + 1)
    }
